...
(() => {
  ...
  /* ============================================================
     ...
     ============================================================ */
  const HERO_LAYERS = [
    { y: 0.28, amp: 16, l1: 560, l2: 350, s1: 0.46, s2: 0.3, fill: "rgba(43,180,166,.10)" },
    {
      y: 0.44,
      amp: 21,
      l1: 440,
      ...
      s1: 0.38,
      s2: 0.52,
      fill: "rgba(43,180,166,.14)",
      stroke: "rgba(191,233,223,.28)",
    },
    {
      y: 0.62,
      amp: 24,
      ...
      s1: 0.58,
      s2: 0.4,
      fill: "rgba(16,64,88,.55)",
      stroke: "rgba(191,233,223,.16)",
    },
    { y: 0.8, amp: 17, l1: 300, l2: 180, s1: 0.72, s2: 0.5, fill: "rgba(6,26,43,.85)" },
  ];
  const DIV_LAYERS = [
    { y: 0.34, amp: 10, l1: 500, l2: 310, s1: 0.4, s2: 0.28, fill: "rgba(43,180,166,.08)" },
    {
      y: 0.52,
      amp: 12,
      l1: 380,
      ...
      s1: 0.5,
      s2: 0.42,
      fill: "rgba(43,180,166,.12)",
      stroke: "rgba(191,233,223,.22)",
    },
    {
      y: 0.7,
      amp: 10,
      ...
      s1: 0.62,
      s2: 0.5,
      fill: "rgba(191,233,223,.05)",
      stroke: "rgba(191,233,223,.12)",
    },
  ];
    ...
    draw(t) {
      ...
      this.layers.forEach((L, i) => {
        ...
        for (let x = 0; x <= w + step; x += step) {
          ...
          const y =
            mid +
            A * 0.62 * Math.sin((u / L.l1) * TAU + t * L.s1 + this.seed + i * 1.7) +
            A * 0.38 * Math.sin((u / L.l2) * TAU - t * L.s2 + this.seed * 1.7 + i * 2.3);
          if (x === 0) open.moveTo(x, y);
          else open.lineTo(x, y);
        }
        const closed = new Path2D(open);
        closed.lineTo(w + step, h);
        closed.lineTo(0, h);
        closed.closePath();
    ...
    if (!hasGSAP() || reduce) {
      ...
      if (veil)
        addEventListener(
          "scroll",
          () => {
            veil.style.opacity = (depthNow() * 0.45).toFixed(3);
          },
          { passive: true },
        );
      return;
  ...
  const cio = new IntersectionObserver(
    (entries) => {
      for (const e of entries) {
        ...
        cio.unobserve(el);
        if (reduce) {
          el.textContent = fmtNum(to, dec, sep);
          continue;
        }
        const dur = 1400,
          t0 = performance.now();
    ...
    const draw = (progress) => {
      ...
      DATA.forEach(([year, val], i) => {
        ...
        if (hVal > 2) {
          ...
          ctx.lineWidth = 2 * dpr;
          ctx.beginPath();
          ctx.moveTo(x + 1, y + dpr);
          ctx.lineTo(x + bw - 1, y + dpr);
          ctx.stroke();
        }
        // value label
        ...
        if (p > 0.92) {
          ctx.fillStyle = `rgba(191,233,223,${((p - 0.92) / 0.08).toFixed(2)})`;
  ...
  if (form) {
    ...
    const PRESET = {
      ...
      25: {
        ...
        monthly:
          "Next session, keep 25 seconds at the door for this memory: you know how to finish.",
      },
      50: {
        once: "Take 50 seconds. Let the page move without you. The queue is clear; there is nowhere to rush.",
        monthly:
          "Next session, take 50 seconds before the first card and bring this calm back with you.",
      },
      100: {
        once: "Take 100 seconds. Stand, stretch, find water, and give this completed session a real ending.",
        monthly:
          "Next session, reserve 100 seconds to arrive slowly and remember the finish already behind you.",
      },
    };
    ...
    const update = () => {
      ...
      donateBtn.textContent = monthly
        ? `Carry ${amt.toLocaleString("en-US")}s forward`
        : `Take ${amt.toLocaleString("en-US")} seconds`;
    ...
    form.addEventListener("submit", (e) => {
      ...
      status.textContent =
        "Moment marked. dailyKpiCompleted is true: all due cards are complete and the daily new-question goal is reached, and this finish is yours.";
