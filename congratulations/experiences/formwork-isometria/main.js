...
function initWorld() {
  ...
  let renderer;
  try {
    renderer = new THREE.WebGLRenderer({ canvas, context: ctx, antialias: true, alpha: true });
  } catch (e) {
    return;
  }
  ...
  scene.add(sun);
  const fill = new THREE.DirectionalLight(0xa8cff, 0.0);
    ...
    for (let j = 0; j < ROWS; j++) {
      ...
      if ((lvl === 2 || lvl === 3) && (i * 7 + j * 3) % 5 === 0) {
        ...
        const leaf = new THREE.Mesh(
          new THREE.ConeGeometry(0.34, 0.85, 7),
          new THREE.MeshLambertMaterial({ color: 0x3f9b57 }),
        );
  ...
  const body = new THREE.Mesh(
    new THREE.CylinderGeometry(0.32, 0.42, 1.4, 10),
    new THREE.MeshLambertMaterial({ color: 0xfdfbf5 }),
  );
  const roof = new THREE.Mesh(
    new THREE.ConeGeometry(0.5, 0.5, 10),
    new THREE.MeshLambertMaterial({ color: 0xef8d5a }),
  );
  ...
  house.add(body, roof, lamp);
  house.traverse((o) => {
    if (o.geometry && o.material.type !== "MeshBasicMaterial") o.castShadow = true;
  });
  ...
  function frame() {
    ...
    if (!reduce)
      trees.forEach((tr) => {
        tr.rotation.z = Math.sin(t * 1.5 + tr.userData.phase) * 0.04;
      });
    if (!reduce) lamp.material.color.setHSL(0.12, 1, 0.6 + Math.sin(t * 3) * 0.12);
    // hover raise
    ray.setFromCamera(ptr, camera);
    const hit = ray.intersectObjects(
      world.children.filter((o) => o.userData.baseY !== undefined),
    )[0];
...
if (btn)
  btn.addEventListener("click", () => {
    ...
    document
      ...
      .setAttribute("content", night ? "#0e1230" : "#bfe9ff");
    setNight(night);
  });
