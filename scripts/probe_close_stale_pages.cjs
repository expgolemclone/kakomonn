...
socket.on("open", async () => {
  try {
    ...
    for (const info of targetInfos) {
      ...
      if (info.url.startsWith("edge://inspect")) continue;
      try {
        await send("Target.closeTarget", { targetId: info.targetId });
        closed += 1;
        console.log("closed:", info.url.slice(0, 90));
      } catch (error) {
        console.log("failed to close:", info.url.slice(0, 90), error.message);
      }
    }
    console.log(`closed ${closed} page targets`);
    socket.close();
    approval.kill();
    process.exit(0);
  } catch (error) {
