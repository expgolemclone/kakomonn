...
for (const target of document.querySelectorAll("[data-milestone]")) {
  ...
  if (target instanceof HTMLDataElement) {
    target.value = String(milestone);
  }
}
for (const target of document.querySelectorAll("[data-milestone-padded]")) {
  target.textContent = paddedMilestone;
}
starMap.setAttribute("aria-label", `${milestone}問達成を表す星座`);
document.title = `${milestone}問達成 | Midnight Study Archive`;
