...
(() => {
  ...
  if (ctx) {
    ...
    const fbm = (x, y, z) => {
      let val = 0,
        ...
        freq = 1;
      for (let i = 0; i < 3; i++) {
        val += amp * vnoise(x * freq, y * freq, z);
        freq *= 2;
        amp *= 0.5;
      }
      return val;
    ...
    const VMIN = 0.06,
      VMAX = 1.16; // iso-value span (matches the field's range)
    const EMIN = 380,
      EMAX = 3180; // metres mapped onto that span
    const N = 17,
      HI = 8; // level count + which one is the marked route
    const levels = [];
    for (let i = 0; i < N; i++) {
    ...
    function marchLevel(L) {
      for (let r = 0; r < rows - 1; r++) {
        ...
        for (let c = 0; c < cols - 1; c++) {
          ...
          const ax = x0 + (sx * (L - tl)) / (tr - tl),
            ay = y0;
          const bx = x1,
            by = y0 + (sy * (L - tr)) / (br - tr);
          const cx2 = x0 + (sx * (L - bl)) / (br - bl),
            cy2 = y1;
          const dx = x0,
            dy = y0 + (sy * (L - tl)) / (bl - tl);
          switch (idx) {
    ...
    function render(z, drift, off) {
      ...
      // minor contours
      ctx.strokeStyle = `rgba(${BARK},0.30)`;
      ctx.lineWidth = 1;
      ctx.beginPath();
      ...
      const anchors = [];
      const texts = [];
      ctx.strokeStyle = `rgba(${MOSS},0.52)`;
      ctx.lineWidth = 1.7;
      for (const lv of levels) {
        ...
        marchLevel(lv.v + off);
        ctx.stroke();
        for (let j = 0; j < cand.length; j += 3) {
          anchors.push(cand[j], cand[j + 1], cand[j + 2]);
          texts.push(lv.elev);
        }
      }
      cand = null;
  ...
  const animateCount = (el) => {
    ...
    const render = (v) =>
      comma ? Math.round(v).toLocaleString("en-US") : Math.round(v).toString();
    if (reduce) {
