...
(() => {
  ...
  if (stage && raw && handle) {
    ...
    if (!reduce) {
      ...
      const drift = () => {
        if (idled) return;
        t += 0.012;
        set(50 + Math.sin(t) * 34);
        requestAnimationFrame(drift);
      };
      setTimeout(() => {
        if (!idled) requestAnimationFrame(drift);
      }, 1400);
    }
  }
  ...
  const computeFilter = (s) => {
    ...
    return `brightness(${bright.toFixed(3)}) contrast(${clamp(contrast, 0.3, 2).toFixed(3)}) saturate(${clamp(sat, 0, 3).toFixed(3)}) sepia(${sepia.toFixed(3)}) hue-rotate(${hue.toFixed(1)}deg)`;
  };
  const apply = () => {
    ...
    const graded =
      Math.abs(state.exposure) +
        Math.abs(state.contrast) +
        Math.abs(state.saturation) +
        Math.abs(state.warmth) >
      24;
  ...
  if (slidersEl) {
    ...
    for (const a of ADJ) {
      ...
      input.addEventListener("input", () => {
        ...
        valEl.textContent = fmt(a, state[a.key]);
        apply();
      });
    }
  }
  const setState = (target, animate) => {
    ...
    if (animate && hasGSAP() && !reduce) {
      ...
      window.gsap.to(proxy, {
        ...
        onUpdate() {
          for (const k of keys) {
            state[k] = proxy[k];
            if (els[k]) {
              els[k].input.value = Math.round(state[k]);
              els[k].valEl.textContent = fmt(els[k].meta, Math.round(state[k]));
            }
          }
          apply();
        },
      ...
      for (const k of keys) {
        state[k] = target[k];
        if (els[k]) {
          els[k].input.value = target[k];
          els[k].valEl.textContent = fmt(els[k].meta, target[k]);
        }
      }
      apply();
    }
  ...
  document
    ...
    ?.addEventListener("click", () =>
      setState(
        { exposure: 10, contrast: 24, shadows: 16, saturation: 15, warmth: 22, grain: 42 },
        true,
      ),
    );
  document
  ...
  if (peek && panelImg) {
    let saved = null;
    const show = () => {
      saved = panelImg.style.filter;
      panelImg.style.filter =
        "saturate(.55) contrast(.82) brightness(1.08) sepia(.06) hue-rotate(-8deg)";
    };
    const hide = () => {
      if (saved !== null) {
        panelImg.style.filter = saved;
        saved = null;
      }
    };
    peek.addEventListener("pointerdown", show);
    peek.addEventListener("pointerup", hide);
    peek.addEventListener("pointerleave", hide);
    peek.addEventListener("blur", hide);
    peek.tabIndex = 0;
  ...
  const LOOKS = [
    {
      n: "FIRST LIGHT",
      f: "saturate(1.05) contrast(1.05) sepia(.12) hue-rotate(-6deg) brightness(1.03)",
      s: { exposure: 6, contrast: 8, shadows: 12, saturation: 6, warmth: 20, grain: 34 },
    },
    {
      n: "DEEP FOCUS",
      f: "saturate(.18) contrast(1.22) brightness(.98)",
      s: { exposure: -4, contrast: 30, shadows: -10, saturation: -90, warmth: 0, grain: 50 },
    },
    {
      n: "STILLNESS",
      f: "saturate(.92) contrast(1.12) sepia(.06) hue-rotate(8deg) brightness(1.0)",
      s: { exposure: 2, contrast: 16, shadows: 20, saturation: -6, warmth: -14, grain: 40 },
    },
    {
      n: "GOLD HOUR",
      f: "saturate(1.2) contrast(1.04) sepia(.2) hue-rotate(-12deg) brightness(1.06)",
      s: { exposure: 12, contrast: 6, shadows: 18, saturation: 20, warmth: 40, grain: 28 },
    },
    {
      n: "CLEAN SLATE",
      f: "saturate(.6) contrast(1.16) brightness(1.08)",
      s: { exposure: 10, contrast: 20, shadows: -6, saturation: -40, warmth: 6, grain: 22 },
    },
    {
      n: "LAST LIGHT",
      f: "saturate(.85) contrast(.9) sepia(.14) brightness(1.08)",
      s: { exposure: 8, contrast: -14, shadows: 30, saturation: -8, warmth: 16, grain: 46 },
    },
  ];
