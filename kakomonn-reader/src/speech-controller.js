export function installSpeechController(app) {
  ...
  async function prepareCorrectFeedbackKpiAudio(remainingPromise, runId) {
    ...
    const request = requestAzureSpeechAudio(token, text, app.ANSWER_RESULT_SPEECH_RATE);
    app.activeSpeechRequests.add(request);
    try {
  ...
  async function playCorrectFeedbackSequence(variant, remainingPromise) {
    ...
    const runId = app.speechRunId;
    cancelActiveSpeech();
    const preparedKpiAudioPromise = prepareCorrectFeedbackKpiAudio(remainingPromise, runId).then(
  ...
  function speakAnswerResult(answerResult) {
    ...
    void (async () => {
      ...
      const runId = app.speechRunId;
      cancelActiveSpeech();
      await playFeedbackAudio(app.FEEDBACK_AUDIO_DATA_URLS.incorrect, runId, label);
    })();
  }
  ...
  Object.defineProperties(app, {
    ...
    prepareCorrectFeedbackKpiAudio: {
      enumerable: false,
      get: () => prepareCorrectFeedbackKpiAudio,
    },
