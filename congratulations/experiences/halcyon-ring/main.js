...
(() => {
  ...
  const breathFrame = (now) => {
    ...
    if (t < 4) {
      phase = 0;
      local = t / 4;
      scale = 1 + AMP * easeInOutSine(local);
    } else if (t < 11) {
      phase = 1;
      local = (t - 4) / 7;
      scale = 1 + AMP + 0.006 * Math.sin(local * Math.PI * 2);
    } else {
      phase = 2;
      local = (t - 11) / 8;
      scale = 1 + AMP * (1 - easeInOutSine(local));
    }
    ...
    if (remaining !== lastCount && breathCount) {
      lastCount = remaining;
