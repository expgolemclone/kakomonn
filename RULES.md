...
- production同期E2Eで`KAKOMONN_SYNC_TOKEN`が未設定の場合は,EdgeとChromeのTampermonkey storageに保存された`kakomonn-reader.sync-token`を特定し,productionの`/v6/sites`で認証を検証してから,ignore済みの`.env`へ保存すること.token値をtool出力,log,回答,tracked fileへ表示または保存しないこと.保存値がproductionで認証されない場合は,secretを勝手にrotateせず,ユーザーへ矛盾を報告すること.
