...
const celebrations = {
  ...
  "forge-fury": {
    ...
    eyebrow: "MILESTONE SECURED / HEAT MAX",
...
const artTemplates = {
  ...
  officer: `
    <div class="scan-grid"></div><div class="case-number">CASE K-<span data-milestone></span></div>
  ...
  archivist: `
    ...
    <div class="archive-slip"><small>RECORD</small><strong data-milestone></strong><span>SEALED</span></div>`,
  gouten: `
  ...
  taiko: `
    ...
    <div class="great-drum"><span data-milestone></span><i></i></div><div class="beat-word">轟</div>`,
  examiner: `
  ...
  forge: `
    ...
    <div class="anvil"><span data-milestone></span></div><div class="clang">CLANG!</div>`,
  complete: `
    ...
    <div class="data-card card-a">RETURNED <b>01</b></div><div class="data-card card-b">FINISHED <b data-milestone></b></div>
...
function renderCelebration(root, config, milestone) {
  const padded = String(milestone).padStart(3, "0");
  ...
  root.innerHTML = `
    ...
    <section class="hero" aria-labelledby="celebration-title">
      <div class="hero-copy">
        ...
        <p class="milestone"><strong data-milestone>${milestone}</strong><span>問達成</span><small>RECORD ${padded}</small></p>
    ...
    <section class="proof" aria-label="達成記録">
      <div class="proof-grid">
        ${config.proof.map((item, index) => `<div><small>0${index + 1}</small><strong>${item}</strong>${index === 1 ? `<span data-milestone>${milestone}</span>` : ""}</div>`).join("")}
      </div>
      <blockquote>${config.quote}</blockquote>
    </section>
    <footer class="site-footer"><span>${config.name}</span><span>${config.license ?? "KEEP GOING / NEXT MILESTONE AWAITS"}</span></footer>`;
  for (const target of root.querySelectorAll("[data-milestone]")) {
...
try {
  ...
  const milestone = parseMilestone(window.location.search, MILESTONE_INTERVAL);
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)");

  renderCelebration(root, config, milestone);
  document.title = `${milestone}問達成 | ${config.name}`;
  ...
  if (window.parent !== window) {
    window.parent.postMessage(
      { type: READY_MESSAGE, siteId, milestone },
