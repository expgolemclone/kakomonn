...
(() => {
  ...
  (function initRolls() {
    ...
    if (reduce) {
      els.forEach((el) => rollTo(el, el.dataset.odo, false));
      return;
    }
    if (hasIO) {
      const io = new IntersectionObserver(
        (entries, obs) => {
          entries.forEach((e) => {
            if (e.isIntersecting) {
              rollTo(e.target, e.target.dataset.odo, true);
  ...
  (function gauge() {
    ...
    const setReadout = (el, v) => {
      if (el && el.firstChild) el.firstChild.nodeValue = v;
    };
    ...
    function draw(bar) {
      ...
      const tr = R + 7 * k;
      ctx.beginPath();
      ctx.arc(c, c, tr, START, START + SWEEP);
      ctx.strokeStyle = "rgba(244,237,228,.10)";
      ctx.lineWidth = 2 * k;
      ctx.stroke();
      // copper progress arc up to the needle
      const frac = bar / MAXBAR,
        nAng = START + frac * SWEEP;
      if (frac > 0.001) {
        ctx.beginPath();
        ctx.arc(c, c, tr, START, nAng);
        ctx.strokeStyle = "#C8622C";
        ctx.lineWidth = 3 * k;
        ctx.lineCap = "round";
        ctx.stroke();
      }
      ...
      ctx.moveTo(-R * 0.16, 0);
      ctx.lineTo(R * 0.9, 0);
      ctx.strokeStyle = "#E98A4B";
      ctx.lineWidth = 2.4 * k;
      ctx.lineCap = "round";
      ctx.shadowColor = "rgba(233,138,75,.55)";
      ctx.shadowBlur = 8 * k;
      ctx.stroke();
      ctx.restore();
      // hub
    ...
    if (reduce) {
      ...
      draw(9);
      setReadout(rTimer, "27.0");
      setReadout(rPressure, "9.0");
      setReadout(rYield, "100");
      return;
    }
  ...
  (function method() {
    ...
    const M = {
      ...
      pourover: {
        ...
        yield: "deeper",
        recipe: [
          "You reached into memory before letting each card move on.",
      ...
      coldbrew: {
        ...
        recipe: [
          ...
          "Then the last due card passed, and dailyKpiCompleted became true after all due cards were complete and the daily new-question goal was reached.",
        ],
      },
    };
    ...
    tabs.forEach((t) =>
      t.addEventListener("click", () => activate(t.dataset.method, { focus: true })),
    );
    tablist.addEventListener("keydown", (e) => {
      ...
      if (n !== null) {
        e.preventDefault();
        activate(tabs[n].dataset.method, { focus: true });
      }
    });
  ...
  (function process() {
    ...
    if (walk) {
      walk.addEventListener("pointerenter", () => {
        paused = true;
      });
      walk.addEventListener("pointerleave", () => {
        paused = false;
      });
    }
    if (hasIO) {
