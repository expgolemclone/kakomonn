...
export function installSyncController(app) {
  ...
  const isAttemptResponse = (value) => isAttemptResponseContract(value, app.SITE_ID);

  const isNextQuestion = (value) => isNextQuestionContract(value, app.SITE_ID);

  const isNextResponse = (value) => isNextResponseContract(value, app.SITE_ID);

  const isCatalogResponse = (value) => isCatalogResponseContract(value, app.SITE_ID);
  ...
  function isSitePageURL(value) {
    try {
      ...
      return url.origin === `https://${app.SITE_ID}` && url.username === "" && url.password === "";
    } catch {
      return false;
    }
  }
  ...
  function extractQuestionIdFromURL(value) {
    try {
      const url = new URL(value);
      if (url.origin !== `https://${app.SITE_ID}` || url.search !== "" || url.hash !== "") {
        return null;
      }
  ...
  function isPendingAttempt(value) {
    ...
    if (value.phase === "queued") {
      return value.nextURL === undefined && value.kpiQuestionsRemaining === undefined;
    }
    const validNextURL =
      value.nextURL === null ||
    ...
    const validKpiQuestionsRemaining =
      value.answerResult === "correct"
        ? Number.isSafeInteger(value.kpiQuestionsRemaining) && value.kpiQuestionsRemaining >= 0
  ...
  function updatePendingAttempt(operationId, update) {
    const task = pendingAttemptStoragePromise.then(async () => {
      if (app.pendingAttempt === null || app.pendingAttempt.operationId !== operationId) {
  ...
  async function markPendingAttemptRecorded(operation, nextURL, kpiQuestionsRemaining) {
    if (
      app.pendingAttempt === null ||
      ...
      (operation.answerResult === "correct" &&
        (!Number.isSafeInteger(kpiQuestionsRemaining) || kpiQuestionsRemaining < 0)) ||
      (operation.answerResult === "incorrect" && kpiQuestionsRemaining !== undefined)
    ) {
      throw new Error("pending attempt changed");
    }
    const updated = await updatePendingAttempt(operation.operationId, (current) => ({
      ...current,
      phase: "recorded",
      nextURL,
      ...(operation.answerResult === "correct" ? { kpiQuestionsRemaining } : {}),
    }));
    if (updated.answerResult === "correct") {
      app.resolveCorrectFeedbackKpi(updated.questionId, updated.kpiQuestionsRemaining);
    }
  }
    ...
    app.syncPromise = (async () => {
      try {
        const nextQuestionResult = app.shouldLaunchNextQuestionAfterSync
          ...
          : null;
        const state = nextQuestionResult?.state ?? (await requestSyncState(candidateToken));
        ...
        app.catalogReady = false;
        app.currentSyncState = null;
        app.syncSettingsError.textContent = `${syncErrorMessage(error)}. ${app.readerErrorDetail(error, "sync-token")}`;
        return false;
      } finally {
        app.syncInProgress = false;
        app.syncPromise = null;
  ...
  Object.defineProperties(app, {
    ...
    pendingAttemptStoragePromise: {
      enumerable: false,
      get: () => pendingAttemptStoragePromise,
      set: (value) => {
        pendingAttemptStoragePromise = value;
      },
    },
