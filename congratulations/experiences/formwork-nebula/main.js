...
function initSky() {
  ...
  const R = 9,
    ...
    RND = 0.42,
    POW = 2.6;
  const inside = new THREE.Color("#ffd9a0"),
    outside = new THREE.Color("#5a6cff");
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    const rx = Math.pow(Math.random(), POW) * (Math.random() < 0.5 ? 1 : -1) * RND * radius;
    const ry = Math.pow(Math.random(), POW) * (Math.random() < 0.5 ? 1 : -1) * RND * radius * 0.4;
    const rz = Math.pow(Math.random(), POW) * (Math.random() < 0.5 ? 1 : -1) * RND * radius;
  ...
  for (let i = 0; i < SCOUNT; i++) {
    ...
    sp[i3] = r * Math.sin(p) * Math.cos(t);
    sp[i3 + 1] = r * Math.cos(p);
    sp[i3 + 2] = r * Math.sin(p) * Math.sin(t);
  }
