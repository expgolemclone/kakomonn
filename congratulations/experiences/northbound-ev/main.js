...
const ICON = {
  ...
  coffee:
    '<svg viewBox="0 0 24 24" fill="none"><path d="M4 9h12v4a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4V9Zm12 1h2a2 2 0 0 1 0 4h-2M6 3v2M9 3v2M12 3v2" stroke="currentColor" stroke-width="1.4"/></svg>',
  trail:
    '<svg viewBox="0 0 24 24" fill="none"><path d="M12 3 4 20h16L12 3Zm0 6-3 7m3-7 3 7" stroke="currentColor" stroke-width="1.4"/></svg>',
  ...
  coastal: {
    ...
    items: [
      { k: "origin", place: "Due queue opened", coord: "FOCUS · CALLED", time: "THEN" },
      {
        ...
        coord: "ATTENTION · ON",
        time: "BEGAN",
        net: "Your focus found the trail",
        grade: "YES",
        adapter: false,
        dwell: "PRESENT",
        meta: ["NOTICED", "ENGAGED", { avail: "NO SHORTCUTS" }, "YOUR EFFORT"],
        tags: [
          { i: "coffee", t: "Met the moment" },
          { i: "walk", t: "Held the line" },
        ],
      },
      {
        k: "stop",
        drive: "Kept the thread · through uncertainty",
        place: "Steady Ground",
        coord: "ATTENTION · HELD",
        time: "FARTHER",
        net: "Distraction did not choose the ending",
        grade: "YES",
        adapter: false,
        dwell: "STAYED",
        meta: ["RETURNED", "CONTINUED", { avail: "ROUTE OPEN" }, "YOUR CHOICE"],
        tags: [
          { i: "coffee", t: "Came back" },
          { i: "view", t: "Saw it through" },
        ],
      },
      {
        k: "dest",
        drive: "Carried focus to the far edge",
        place: "dailyKpiCompleted",
  ...
  alpine: {
    ...
    items: [
      { k: "origin", place: "Due queue opened", coord: "THE ROUTE · BEGAN", time: "THEN" },
      {
        ...
        place: "Attention Pass",
        coord: "FOCUS · HELD",
        time: "MIDWAY",
        net: "Focus held through the climb",
        grade: "YES",
        adapter: false,
        dwell: "STAYED",
        ...
        tags: [
          { i: "coffee", t: "Met the moment" },
          { i: "trail", t: "Kept the line" },
        ],
      },
      {
        k: "stop",
        drive: "Returned after uncertainty",
        place: "Recall Ridge",
        coord: "MEMORY · RETRIEVED",
        time: "HIGH GROUND",
        net: "Memory brought back into reach",
        grade: "YES",
        adapter: true,
        dwell: "RETURNED",
        meta: ["RECALLED", "RETRIED", { avail: "KEPT GOING" }, "YOUR WORK"],
        tags: [
          { i: "coffee", t: "Found the memory" },
          { i: "fast", t: "Turn completed" },
        ],
      },
      {
        k: "dest",
  ...
  desert: {
    ...
    items: [
      { k: "origin", place: "Due queue opened", coord: "RESOLVE · TESTED", time: "THEN" },
      {
        ...
        coord: "PATIENCE · HELD",
        time: "EARLY",
        net: "Difficulty became part of the route",
        grade: "YES",
        adapter: true,
        dwell: "PAUSED",
        meta: ["NOT EASY", "STILL HERE", { avail: "PATH FOUND" }, "YOUR PATIENCE"],
        tags: [
          { i: "coffee", t: "Made room" },
          { i: "view", t: "Stayed honest" },
        ],
      },
      {
        k: "stop",
        drive: "Tried the path again",
        place: "Return Point",
        coord: "EFFORT · RENEWED",
        time: "MIDWAY",
        net: "A second pass opened the view",
        grade: "YES",
        adapter: false,
        dwell: "RETRIED",
        ...
        tags: [
          { i: "coffee", t: "Came back" },
          { i: "trail", t: "Found a way" },
        ],
      },
      {
        k: "stop",
        drive: "Met the final stretch",
        place: "Last Ridge",
        coord: "RESOLVE · INTACT",
        time: "NEARLY THERE",
        net: "The unfinished edge disappeared",
        grade: "YES",
        adapter: false,
        dwell: "FINISHED",
        meta: ["NO RUSH", "NO RETREAT", { avail: "HORIZON CLEAR" }, "YOUR RESOLVE"],
        tags: [
          { i: "view", t: "Saw the end" },
          { i: "walk", t: "Crossed it" },
        ],
      },
      {
...
const nodeHTML = (it) => {
  ...
  const meta = it.meta
    .map((m) =>
      typeof m === "string" ? `<span>${m}</span>` : `<span class="avail">${m.avail}</span>`,
    )
    .join("");
  const tags = it.tags
    .map(
      (t) =>
        `<span class="tag ${t.i === "fast" ? "fast" : ""}">${ICON[t.i] || ICON.trail}${t.t}</span>`,
    )
    .join("");
  const badges =
    `<span class="stopcard-badge grade">${it.grade}</span>` +
    (it.adapter ? '<span class="stopcard-badge adapter">Turn met</span>' : "");
...
const renderRoute = (key) => {
  ...
  set("sumKm", r.km);
  set("sumStops", r.stops);
  set("sumArrive", r.arrive);
  set("odFrom", r.from);
  set("odFromCoord", r.fromC);
  set("odTo", r.to);
  set("odToCoord", r.toC);
  const stats = $("#sumStats");
  if (stats)
    stats.innerHTML = r.stats
      .map(([v, l]) => `<div class="ss"><b>${v}</b><span>${l}</span></div>`)
      .join("");
  const tl = $("#timeline");
  if (tl) tl.innerHTML = r.items.map(nodeHTML).join("");
  // hero chip mirrors the featured summary
  const chipLead = $(".route-chip .rc-lead b"),
    chipSpan = $(".route-chip .rc-lead span");
  ...
  if (ringRevealed) setRing(r.batt);
  else if (ringPct) ringPct.textContent = r.batt + "%";
  const rrSoc = $("#rrSoc");
  if (rrSoc) rrSoc.textContent = r.batt + "%";
...
async function initTerrain() {
  ...
  const nodes = NODE_U.map((u) => {
    ...
    const core = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0xdfffdd }),
    );
  ...
  window.__nbTerrain = {
    setStops: (n) => {
      activeStops = clamp(n, 1, NODE_U.length);
    },
  };
  addEventListener(
    "pagehide",
    () => {
      ...
      glowTex.dispose();
      nodes.forEach((n) => {
        n.userData.core.geometry.dispose();
        n.userData.core.material.dispose();
