...
const { configureSyncToken, extractBuildFingerprint } = require("./live_sync_e2e_test");
...
async function main() {
  ...
  try {
    ...
    const configured = await configureSyncToken(setupPage, token, expectedBuildFingerprint);
    ...
    assert.equal(
      await dashboardPage.locator("html").getAttribute("data-kakomonn-dashboard-bridge-state"),
      "ready",
    );
    assert.match(
    ...
    assert.equal(await dashboardPage.locator("#auth-token, #settings-dialog").count(), 0);
    assert.equal(
      await dashboardPage.evaluate(() => localStorage.getItem("kakomonn-dashboard.sync-token")),
      null,
    );
    assert.equal(
      await dashboardPage
        .locator("html")
        .evaluate((element, secret) => element.outerHTML.includes(secret), token),
      false,
    );
    console.log(
      JSON.stringify({
