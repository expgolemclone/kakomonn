...
const CAT = [
  {
    id: "M42",
    name: "First Spark",
    cat: "M42 · OPENING LIGHT",
    type: "A bright beginning · Today",
    kind: "emission",
    ra: 5.588,
    raS: "05ʰ 35ᵐ 17ˢ",
    decS: "−05° 23′ 28″",
  ...
  },
  {
    id: "M45",
    name: "Small Wins",
    cat: "M45 · BRIGHT STEPS",
    type: "Progress cluster · Today",
    kind: "cluster",
    ra: 3.79,
    raS: "03ʰ 47ᵐ 24ˢ",
    decS: "+24° 07′ 00″",
  ...
  },
  {
    id: "M13",
    name: "Steady Core",
    cat: "M13 · FULL MOMENTUM",
    type: "Sustained effort · Today",
    kind: "globular",
    ra: 16.695,
    raS: "16ʰ 41ᵐ 41ˢ",
    decS: "+36° 27′ 37″",
  ...
  },
  {
    id: "NGC7000",
    name: "Clear Horizon",
    cat: "NGC 7000 · OPEN SKY",
    type: "Completion glow · Today",
    kind: "emission",
    ra: 20.978,
    raS: "20ʰ 58ᵐ 47ˢ",
    decS: "+44° 19′ 48″",
  ...
  },
  {
    ...
    name: "Finish Line",
    cat: "M8 · ALL CLEAR",
    type: "Final bright moment · Today",
    kind: "emission",
    ra: 18.061,
    raS: "18ʰ 03ᵐ 41ˢ",
    decS: "−24° 23′ 00″",
  ...
  },
  {
    id: "NGC253",
    name: "Afterglow",
    cat: "NGC 253 · WELL EARNED",
    type: "Quiet celebration · Today",
    kind: "galaxy",
    ra: 0.793,
    raS: "00ʰ 47ᵐ 33ˢ",
    decS: "−25° 17′ 18″",
...
function glint(x, y, size, col) {
  return `<g opacity=".9"><circle cx="${x}" cy="${y}" r="${size * 1.6}" fill="${col}" opacity=".16"/>
    <circle cx="${x}" cy="${y}" r="${size * 0.5}" fill="#fff"/>
...
function makeSky(o) {
  ...
  if (o.kind === "emission") {
    ...
    for (let i = 0; i < dense; i++) {
      ...
      const rad = o.kind === "globular" ? Math.pow(rnd(), 1.7) * 110 : Math.pow(rnd(), 0.8) * 130;
      const x = (200 + Math.cos(a) * rad + (rnd() - 0.5) * 20).toFixed(1);
      const y = (
        150 +
        Math.sin(a) * rad * (o.kind === "globular" ? 1 : 0.8) +
        (rnd() - 0.5) * 20
      ).toFixed(1);
      const r = (0.5 + rnd() * 1.4).toFixed(2);
      const tint = rnd() < 0.2 ? "#bfe0ff" : rnd() < 0.15 ? "#ffe6c8" : "#e8eefc";
      body += `<circle cx="${x}" cy="${y}" r="${r}" fill="${tint}" opacity="${(0.4 + rnd() * 0.55).toFixed(2)}"/>`;
    }
  }

  const stars = starDots(rnd, 90, 1.1);
  const glints =
    glint(60 + rnd() * 280, 40 + rnd() * 220, 2.2, "#9fe9dd") +
...
(function observations() {
  ...
  if (!grid) return;
  const items = ["M42", "M31", "M45", "M51", "M27", "NGC7000"].map((id) =>
    CAT.find((o) => o.id === id),
  );
...
const NODES = [
  {
    ...
    ap: "0.6 m RC",
    status: "live",
    target: "dailyKpiCompleted · confirmed",
  },
  {
    name: "Pacific Chorus",
    loc: "Hawaiʻi, USA",
    ...
    ap: "0.5 m Newt",
    status: "slew",
    target: "sending → one more cheer",
  },
  {
    name: "Canary Glow",
    loc: "Canary Islands, ES",
    lat: "+28.7606°",
    lon: "−17.8814°",
    el: "2396 m",
    see: "1.0″",
    ap: "0.4 m APO",
    status: "live",
    target: "today · beautifully complete",
  },
  {
    name: "Karoo Spark",
    loc: "Sutherland, ZA",
    ...
    ap: "0.7 m CDK",
    status: "live",
    target: "your finish · recorded",
  },
  {
    name: "Southern Quiet",
    loc: "Coonabarabran, AU",
    lat: "−31.2755°",
    lon: "+149.067°",
    el: "1165 m",
    see: "1.6″",
    ap: "0.35 m Newt",
    status: "idle",
    target: "resting · daylight",
  },
  {
    name: "Highland Echo",
    loc: "Aviemore, Scotland",
    lat: "+57.1200°",
    lon: "−3.8300°",
    el: "620 m",
    see: "2.4″",
    ap: "0.3 m APO",
    status: "cloud",
    target: "soft glow · clouds passing",
  },
];
const STATUS = {
  live: ["BRIGHT", "st-live"],
  slew: ["ARRIVING", "st-slew"],
  idle: ["RESTING", "st-idle"],
  cloud: ["VEILED", "st-cloud"],
};
(function network() {
  ...
  const cyc = [
    "sending → one more cheer",
    "crossing the Pacific",
    "signal steady · 0.42″",
    "cheer received · bright",
  ];
  let k = 0,
    timer = null;
  const tick = () => {
    k = (k + 1) % cyc.length;
    if (el) el.textContent = cyc[k];
  };
  const run = () => {
    if (!timer && !document.hidden) timer = setInterval(tick, 4200);
  };
  const stop = () => {
    if (timer) {
      clearInterval(timer);
...
(function skyConsole() {
  ...
  const D = {
    ...
    lock: document.getElementById("dLock"),
    name: document.getElementById("dName"),
    type: document.getElementById("dType"),
    ra: document.getElementById("dRa"),
    dec: document.getElementById("dDec"),
    alt: document.getElementById("dAlt"),
    az: document.getElementById("dAz"),
    mag: document.getElementById("dMag"),
    size: document.getElementById("dSize"),
    node: document.getElementById("dNode"),
    status: document.getElementById("vpStatus"),
  ...
  const bg = Array.from({ length: 420 }, () => ({
    ...
    o: 0.2 + rnd() * 0.6,
    ph: rnd() * Math.PI * 2,
    tint: rnd() < 0.1 ? "#7fe6d6" : rnd() < 0.1 ? "#b0a6ff" : "#e8eefc",
  }));
  ...
  function updateDetail(o) {
    if (!o) {
      D.status.textContent = "SEEKING";
      D.card.classList.remove("locked");
      D.lock.textContent = "NO SPARK";
      D.cat.textContent = "— · —";
      D.name.textContent = "Sweep to revisit a bright moment";
      D.type.textContent =
        "Move the reticle through the field and let one part of today come into focus.";
      D.ra.textContent =
        D.dec.textContent =
        D.alt.textContent =
        D.az.textContent =
    ...
    D.card.classList.add("locked");
    D.lock.textContent = "MOMENT FOUND";
    D.cat.textContent = o.cat;
    D.name.textContent = o.name;
    D.type.textContent = o.type;
    D.ra.textContent = o.raS;
    D.dec.textContent = o.decS;
    D.alt.textContent = (alt >= 0 ? "+" : "−") + Math.abs(alt).toFixed(1) + "°";
    D.az.textContent = az.toFixed(1) + "° " + compass(az);
    D.mag.textContent = o.mag;
    D.size.textContent = o.size;
  ...
  function draw(t) {
    ...
    // faint grid
    ctx.strokeStyle = "rgba(232,238,252,0.04)";
    ctx.lineWidth = 1;
    for (let i = 1; i < 6; i++) {
      ctx.beginPath();
      ctx.moveTo((W / 6) * i, 0);
    ...
    for (const o of objs) {
      const col =
        o.kind === "galaxy" ? "124,108,255" : o.kind === "emission" ? "67,224,200" : "190,224,255";
      ...
      ctx.fillStyle = g;
      ctx.beginPath();
      ctx.arc(o.x, o.y, 16, 0, 7);
      ctx.fill();
      ctx.fillStyle = "#eef4ff";
      ctx.beginPath();
      ctx.arc(o.x, o.y, 1.6, 0, 7);
      ctx.fill();
      if (locked === o) {
    ...
    [
      ...
      ctx.moveTo(rx + sx * b, ry + sy * (b - 8));
      ctx.lineTo(rx + sx * b, ry + sy * b);
      ctx.lineTo(rx + sx * (b - 8), ry + sy * b);
    });
    ctx.stroke();
    // crosshair gap
    ctx.beginPath();
    ctx.moveTo(rx - rr - 6, ry);
    ctx.lineTo(rx - 5, ry);
    ctx.moveTo(rx + 5, ry);
    ctx.lineTo(rx + rr + 6, ry);
    ctx.moveTo(rx, ry - rr - 6);
    ctx.lineTo(rx, ry - 5);
    ctx.moveTo(rx, ry + 5);
    ctx.lineTo(rx, ry + rr + 6);
    ctx.stroke();
    // plate-solve scan sweep on fresh lock
    if (on && !reduce && t - lockT < 620) {
      const p = (t - lockT) / 620;
      ctx.globalAlpha = (1 - p) * 0.8;
      ctx.strokeStyle = "rgba(67,224,200,0.9)";
      ctx.beginPath();
      ctx.arc(rx, ry, 10 + p * 40, 0, 7);
      ctx.stroke();
      ctx.globalAlpha = 1;
    }
  }
...
/* ---------------------------------------------------------
   ...
   --------------------------------------------------------- */
(function tonight() {
  const obs = document.getElementById("tObs"),
    lst = document.getElementById("tLst"),
    see = document.getElementById("tSeeing"),
    tr = document.getElementById("tTransit");
  if (!lst) return;
  const fmtLst = (h) => {
    const H = h | 0,
      M = ((h - H) * 60) | 0,
  ...
  const transiting = () => {
    ...
    let best = CAT[0],
      bd = 99;
    for (const o of CAT) {
      let d = Math.abs(((cur - o.ra + 12 + 24) % 24) - 12);
      if (d < bd) {
        bd = d;
        best = o;
      }
  ...
  let base = 890 + ((Math.random() * 60) | 0),
    seeBase = 1.8;
  const tick = () => {
    lst.textContent = fmtLst(localSiderealHours());
    if (tr) {
      const o = transiting();
      tr.textContent = `${o.id} · ${o.name}`;
    }
  };
  tick();
  if (reduce) {
    if (obs) obs.textContent = String(base);
    return;
  }
  let t0 = null;
  const soft = (ts) => {
    ...
    const e = (ts - t0) / 1000;
    if (obs)
      obs.textContent = String(base + Math.round(Math.sin(e * 0.25) * 22 + Math.sin(e * 1.7) * 4));
...
(function invite() {
  const form = document.getElementById("inviteForm"),
    input = document.getElementById("inviteEmail"),
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v)) {
      note.textContent = "Enter a valid address — this demo will still send nothing.";
      note.classList.remove("ok");
      input.focus();
      return;
    }
    note.textContent =
      "✓ Moment held. Nothing was sent or stored — this little afterglow is yours.";
  ...
  import("three")
    .then((THREE) => {
      ...
      const palette = [
        [0.91, 0.93, 0.99],
        [0.91, 0.93, 0.99],
        [0.91, 0.93, 0.99], // white-blue (majority)
        [0.72, 0.82, 1.0],
        [0.6, 0.78, 1.0], // blue
        [0.26, 0.88, 0.78], // teal
        [0.55, 0.47, 1.0], // violet
        [1.0, 0.86, 0.66], // warm
      ];
    ...
    .catch(() => {
      /* CDN unavailable — CSS starfield fallback stays */
    });
}
