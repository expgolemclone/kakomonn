...
(() => {
  ...
  const syncTheme = () => {
    ...
    document
      ...
      .setAttribute("content", dark ? "#05141c" : "#eef2f0");
  };
  syncTheme();
  themeBtn?.addEventListener("click", () => {
    root.dataset.theme = root.dataset.theme === "light" ? "dark" : "light";
    try {
      localStorage.setItem("fathom-theme", root.dataset.theme);
    } catch (e) {}
    syncTheme();
  });
  ...
  const ZONE_NAME = {
    sunlight: "First light",
    twilight: "The current",
    midnight: "Deep work",
    abyss: "The finish",
  };
  ...
  if (form) {
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      const name = form.querySelector("#tName"),
        email = form.querySelector("#tEmail"),
        date = form.querySelector("#tDate");
      for (const el of [name, email, date]) {
        if (!el.checkValidity()) {
          el.reportValidity();
          return;
        }
  ...
  const vso = compile(gl.VERTEX_SHADER, vs),
    fso = compile(gl.FRAGMENT_SHADER, fs);
  if (!vso || !fso) {
    canvas.style.background = FALLBACK;
    return;
  }
