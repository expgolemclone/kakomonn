...
- FSRS stabilityが30日以上になった定着問題数と解答履歴. 値は`kakomonn-sync`へ保存され,Win11 EdgeとiPhone Safariで共有されます.問題画面では定着問題数と今日解いた問題数を確認できます.同期Workerのrootでは定着問題数,今日の定着純増,解いた問題数,直近7日間の定着数と日別解答問題数を確認できます.解いた問題数は問題IDの種類数で,同じ問題を同じ日に繰り返しても1問として数えます.
