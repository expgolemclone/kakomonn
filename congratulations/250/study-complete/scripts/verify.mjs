...
async function openPage({ viewport, reducedMotion }) {
  ...
  await page.goto(`${server.origin}/250/study-complete/?milestone=250`, {
...
try {
  ...
  const desktop = await verifyStatic({
    viewport: { width: 1440, height: 1000 },
    screenshot: resolve('250/study-complete/preview.png'),
  });
  const mobile = await verifyStatic({
    viewport: { width: 390, height: 844 },
    screenshot: resolve('250/study-complete/preview-mobile.png'),
  });
