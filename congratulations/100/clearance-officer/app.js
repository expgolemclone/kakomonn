...
const officerZone = document.querySelector(".officer-zone");

if (
  !(eye instanceof HTMLElement) ||
  !(replay instanceof HTMLButtonElement) ||
  !(confetti instanceof HTMLElement) ||
  !(sweep instanceof HTMLElement) ||
  !(officerZone instanceof HTMLElement)
) {
  throw new Error("Clearance Officer UI nodes are missing.");
}

const milestone = parseMilestone(window.location.search, 50);
const paddedMilestone = String(milestone).padStart(3, "0");
for (const target of document.querySelectorAll("[data-milestone]")) {
  target.textContent = String(milestone);
}
for (const target of document.querySelectorAll("[data-milestone-code]")) {
  target.textContent = `K-${milestone}`;
}
for (const target of document.querySelectorAll("[data-document-code]")) {
  target.textContent = `DOC // CB-K${milestone}-${paddedMilestone}`;
}
officerZone.setAttribute("aria-label", `祝賀監査官 K-${milestone}`);
document.title = `CHECKPOINT CLEARED // K-${milestone}`;
